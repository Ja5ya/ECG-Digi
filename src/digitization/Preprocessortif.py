...
class Preprocessor:
    ...
    def __gridline_removal(self, ecg: Image) -> Image:
        ...
        if len(ecg.data.shape) == 3 and ecg.data.shape[2] == 3:  # Check if the image is BGR
            gray_image = cv.cvtColor(ecg.data, cv.COLOR_BGR2GRAY)
        else:
            gray_image = ecg.data

        # Create mask for gridlines
        hsv_image = cv.cvtColor(ecg.data, cv.COLOR_BGR2HSV)
        ...
        # OTSU binarization
        _, binarized_image = cv.threshold(masked_image, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)

        ecg.data = binarized_image
    ...
    def __outline_borders(self, ecg: Image) -> Image:
        ...
        if len(ecg.data.shape) == 3 and ecg.data.shape[2] == 3:
            gray_image = cv.cvtColor(ecg.data, cv.COLOR_BGR2GRAY)
        else:
            gray_image = ecg.data
