...
class Preprocessor:
    ...
    def __img_partitioning(self, ecg: Image) -> Rectangle:
        ...
        adaptive_thresh = cv.adaptiveThreshold(
            blurred_image, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C,
            cv.THRESH_BINARY_INV, 11, 2
        )

        # Morphological operations to close gaps and enhance features
        ...
        if not rects:
            return Rectangle(Point(0, 0), Point(ecg.width, ecg.height))
